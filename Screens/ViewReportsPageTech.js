...
const ViewReportsPageTech = () => {
  ...
  const fetchReports = () => {
    ...
    const unsubscribe = onSnapshot(query, (snapshot) => {
      ...
      setReports(fetchedReports.filter(report => report.technicianEmail === userEmail)); // Filtrar por el correo electrónico del técnico actual
      setIsLoading(false);
    });
  ...
  useEffect(() => {
    ...
    const reportsQuery = query(reportsCollection, where('technicianEmail', '==', userEmail));  // Asegúrate de que 'userEmail' es el correo del técnico actual.
  ...
  <Text style={styles.header}>Reportes Enviados</Text>
      ...
      <ScrollView style={styles.reportsContainer}>
        {reports.map((report) => (
          <TouchableHighlight key={report.id} style={styles.reportItem} underlayColor="#DDDDDD" onPress={() => navigation.navigate('ReportDetailsPageTech', { report })}>
...
export default ViewReportsPageTech;
