...
const ViewReportsPageTech = () => {
  ...
  const fetchReports = () => {
    ...
    const unsubscribe = onSnapshot(query, (snapshot) => {
      let fetchedReports = snapshot.docs
        .map((doc) => ({ id: doc.id, ...doc.data() }))
        .filter(
          (report) =>
            (!searchQuery || report.folio.toString().includes(searchQuery)) &&
            (dateTitle === "Fecha y Hora" || report.date === formatDate(date))
        );
      setReports(
        fetchedReports.filter((report) => report.technicianEmail === userEmail)
      );
      setIsLoading(false);
    });
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={handleRefresh} style={styles.refreshButton}>
        <Text style={styles.refreshButtonText}>Borrar Filtros</Text>
      </TouchableOpacity>
      <View style={styles.reportHeaders}>
        ...
        >
          <Text style={styles.headerText}>{dateTitle}</Text>
        </TouchableOpacity>
        {showDatePicker && (
          <DateTimePicker
            value={date}
        ...
        {reports.map((report) => (
          ...
          >
            <View style={styles.reportRow}>
              ...
              <Text style={styles.reportColumnText}>
                {report.date} - {report.time}
              </Text>
              <Text
                style={[
                  styles.reportColumnText,
