...
import { Modal, StatusBar, View, Text, TextInput, TouchableOpacity, StyleSheet, Image, Alert, ActivityIndicator, ScrollView } from 'react-native';
...
const images = [{
  ...
  const handleSendReport = async () => {
    ...
    try {
      ...
      // Obtener el nuevo folio
      const db = getFirestore();
      const q = query(collection(db, 'reports'), orderBy('folio', 'desc'), limit(1));
      ...
      // Construir los datos del informe
      const reportData = {
        date: currentDate,
        time: currentTime,
        area: selectedArea,
      ...
      // Limpiar los campos después de enviar el informe
      ...
      Alert.alert('Reporte enviado', `El reporte se ha enviado correctamente. Folio: ${newFolio}`, [
        { text: 'OK' },
      ]);
    } catch (error) {
  ...
  <>
    ...
    <Modal
      ...
      <View style={styles.centeredView}>
        ...
        />
        {errors.no_card && <Text style={styles.errorText}>Este campo es requerido</Text>}
        <Text style={styles.label}>Datos del Equipo</Text>
        <TextInput
          placeholder="Datos del Equipo"
          value={computerData}
          onChangeText={setDatosDelEquipo}
          style={[styles.inputLarge, errors.computerData && styles.errorBorder]}
          multiline
        />
        {errors.computerData && <Text style={styles.errorText}>Este campo es requerido</Text>}
        <Text style={styles.label}>Descripción del Problema</Text>
        <TextInput
          placeholder="Descripción del Problema"
          value={servicePerformed}
          onChangeText={setServicioARealizar}
          style={[styles.inputLarge, errors.servicePerformed && styles.errorBorder]}
          multiline
        />
        {errors.servicePerformed && <Text style={styles.errorText}>Este campo es requerido</Text>}
...
previewImage: {
  ...
  height: 200,   // Altura adecuada para ver la imagen
