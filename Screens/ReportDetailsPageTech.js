...
const pickImage = async () => {
  ...
  const unsubscribeComments = onSnapshot(
    ...
    (snapshot) => {
      ...
      })).sort((a, b) => a.createdAt-b.createdAt); // Orden inverso para que los más recientes estén al principio
  ...
  const unsubscribeReport = onSnapshot(doc(db, "reports", report.id), (doc) => {
    ...
    setReportState(updatedReport); // Actualiza también el estado del reporte que estás utilizando
  }, (error) => {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.commentSection}>
        ...
        {comments.map((commentObj, index) => (
          <View key={index} style={commentObj.userType === 'Técnico' ? styles.commentBubbleRight : styles.commentBubble}>
