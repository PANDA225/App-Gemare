...
const NotificationPageAdmin = () => {
  ...
  useEffect(() => {
    ...
    const reportsQuery = onSnapshot(reportsCollection, (snapshot) => {
        const fetchedReports = snapshot.docs
          .map(doc => {
            const data = doc.data();
            const notificationTime = data.notificationTime ? new Date(data.notificationTime.seconds * 1000) : null;
            return { id: doc.id, ...data, notificationTime };
          })
          .filter(report => report.notification === true); // Filtrar los informes con notificación activa
  ...
  const handleNotificationPress = async (report) => {
    navigation.navigate('ReportDetailsPageAdmin', { report });
