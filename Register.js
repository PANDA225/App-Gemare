...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, StatusBar, Image, ActivityIndicator, ScrollView } from 'react-native';
...
const RegisterScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchAreas = async () => {
      try {
        ...
        const areasData = areasSnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().area }));
        setAreas(areasData);
      } catch (error) {
    ...
    createUserWithEmailAndPassword(auth, email, password)
      .then((userCredentials) => {
        ...
        const userData = {
          ...
          ...(userType === 'Tecnico' && { technicianService: technicianService }),
          ...(userType === 'Usuario' && { userName: userName }),
          ...(userType === 'Usuario' && selectedArea && { area: selectedArea.name }),
        };
  ...
  const renderTechnicianNameInput = () => (
    <View style={styles.pickerContainerText}>
      ...
      <TextInput
        placeholder="Introduce el servicio que realiza"
